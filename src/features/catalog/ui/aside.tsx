    ...
    return (
        <aside className="w-full h-fit p-[0.75rem] rounded-[1.188rem] border lg:p-[1.125rem] bg-white">
            <div className="flex items-center justify-between pb-4">
                ...
                <div className="flex items-center gap-[0.375rem] text-primary">
                    <Image
            ...
            <Accordion type="multiple" defaultValue={Object.keys(catalogFilters)}>
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div
                                            ...
                                            style={{
                                                left: `${((priceRange[0] - 800) / (7200 - 800)) * 100
                                                    }%`,
                                            }}
                                        />
                                    </TooltipTrigger>
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div
                                            className="absolute left-0 top-0 h-6 w-6 -translate-x-1/2 cursor-pointer"
                                            style={{
                                                left: `${((priceRange[1] - 800) / (7200 - 800)) * 100
                                                    }%`,
                                            }}
                                        />
                                    </TooltipTrigger>
                ...
                {Object.entries(catalogFilters).map(([category, options]) => (
                    <FilterAccordion key={category} category={category} options={options} />
                ))}
            </Accordion>
            <Button variant={'default'} className="w-full rounded-full h-[2.688rem] font-bold leading-[112%] text-[0.938rem]">
                Применить
            </Button>
        </aside>
    );
                                    ...
                                    <div className="flex flex-col gap-4 pt-2">
                                        {options.map((option) => (
                                            <div key={option.id} className="flex items-center gap-2.5">
                                                ...
                                                >
                                                    {option.title}{" "}
                                                    <span className="text-[#C4C4C4]">({option.count})</span>
                                                </label>
                                            </div>
                                        ))}
                                    </div>
