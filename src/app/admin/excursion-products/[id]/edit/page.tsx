...
export default function EditExcursionProductPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  const fetchProduct = async () => {
    try {
      ...
      setValue("tickets", formattedData.tickets);
      setValue("dateRanges", formattedData.dateRanges);
      setValue("meetingPoints", formattedData.meetingPoints);
      setValue("paymentOptions", formattedData.paymentOptions);
      setValue("groups", formattedData.groups);
      setValue("isPublished", formattedData.isPublished);
    } catch (error) {
  ...
  const onSubmit = async (data: ProductFormData) => {
    try {
      ...
      router.push(`/admin/excursion-products/${params.id}`);
    } catch (error) {
                ...
                <div key={index} className="flex gap-4 items-start">
                  ...
                  <div className="flex-1">
                    ...
                    {errors.tickets?.[index]?.price && (
                      <p className="text-sm text-red-500 mt-1">
                        {errors.tickets[index]?.price?.message?.toString()}
                      </p>
                    )}
                  </div>
                  <Button
                ...
                <div key={index} className="flex gap-4 items-start">
                  ...
                  <div className="flex-1">
                    ...
                    {errors.meetingPoints?.[index]?.time && (
                      <p className="text-sm text-red-500 mt-1">
                        {errors.meetingPoints[index]?.time?.message?.toString()}
                      </p>
                    )}
                  </div>
                  <Button
                ...
                <div key={index} className="flex gap-4 items-start">
                  <div className="flex-1">
                    ...
                    {errors.groups?.[index]?.minSize && (
                      <p className="text-sm text-red-500 mt-1">
                        {errors.groups[index]?.minSize?.message?.toString()}
                      </p>
                    )}
                  </div>
                  <div className="flex-1">
                    ...
                    {errors.groups?.[index]?.maxSize && (
                      <p className="text-sm text-red-500 mt-1">
                        {errors.groups[index]?.maxSize?.message?.toString()}
                      </p>
                    )}
                  </div>
                  <div className="flex-1">
                    ...
                    {errors.groups?.[index]?.price && (
                      <p className="text-sm text-red-500 mt-1">
                        {errors.groups[index]?.price?.message?.toString()}
                      </p>
                    )}
                  </div>
                  <Button
